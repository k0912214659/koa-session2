...
describe('koa-session2', () => {
  describe('when use default store', () => {
    ...
    it('should work when session expired', function (done) {
      ...
      setTimeout(() => {
        request(server)
          .get('/getSession')
          .set('cookie', cookie)
          .expect(200, (err, res) => { // eslint-disable-line
    ...
    it('should work when multiple clients access', done => {
      request(server)
        .get('/setSession')
        .end((err_1, res_1) => { // eslint-disable-line
          let cookie_1 = res_1.header['set-cookie'] // eslint-disable-line
          ...
          request(server)
            .get('/setSession')
            .end((err_2, res_2) => { // eslint-disable-line
              let cookie_2 = res_2.header['set-cookie'] // eslint-disable-line

              if (cookie_1 != cookie_2) done() // eslint-disable-line
            })
        })
    })
